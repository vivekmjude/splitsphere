...
export const useFriendsStore = create<FriendsState>((set, get) => ({
  ...
  addFriend: async (friend) => {
    try {
      ...
      if (!isUnique) {
        throw new Error('Failed to generate a unique friend ID after multiple attempts');
      }
      ...
      // Add to sync queue
      await addToSyncQueue('friends', 'add', friendToAdd as unknown as Record<string, unknown>);
  ...
  updateFriendStatus: async (friendId, status) => {
    try {
      ...
      if (currentFriend) {
        ...
        await addToSyncQueue('friends', 'update', updatedFriend as unknown as Record<string, unknown>);
      }
  ...
  removeFriend: async (friendId) => {
    try {
      ...
      // Add to sync queue
      if (friend) {
        await addToSyncQueue('friends', 'delete', { id: friendId } as unknown as Record<string, unknown>);
      }
