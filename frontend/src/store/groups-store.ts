...
import { User, Currency } from "@/types/expense";
...
interface GroupsState {
  ...
  recalculateAllGroupBalances: () => Promise<void>;
}
...
export const useGroupsStore = create<GroupsState>((set, get) => ({
  ...
  loadGroups: async () => {
    ...
    try {
      ...
      // We don't need to recalculate balances here anymore
      // This is now handled in the expense store's loadExpenses function
    } catch (error) {
  ...
  addGroup: async (group) => {
    try {
      ...
      await addToSyncQueue(
        "groups",
        "add",
        groupToAdd as unknown as Record<string, unknown>
      );
    ...
    try {
      ...
      if (currentGroup) {
        ...
        await addToSyncQueue(
          "groups",
          "update",
          updatedGroup as unknown as Record<string, unknown>
        );
      }
  ...
  removeGroup: async (groupId) => {
    try {
      ...
      // Add to sync queue
      if (group) {
        await addToSyncQueue("groups", "delete", {
          id: groupId,
  ...
  recalculateAllGroupBalances: async () => {
    try {
      // Get all groups and expenses
      const { groups } = get();
      const { expenses } = await import("@/store/expense-store").then(
        (module) => module.useExpenseStore.getState()
      );
      ...
      for (const group of groups) {
        ...
        // Update the group
        await get().updateGroup(group.id, { totalBalance: totalAmount });
      }

      console.log("All group balances recalculated");
    } catch (error) {
      console.error("Failed to recalculate all group balances:", error);
    }
  },
}));
